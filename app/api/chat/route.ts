...
export async function POST(req: Request) {
  ...
  const memorySource = (snippets && snippets.length > 0 ? snippets : fallback.data ?? []) as Array<{ snippet: string }>;
  const memory = memorySource.map((s: { snippet: string }) => s.snippet).join("\n---\n");
